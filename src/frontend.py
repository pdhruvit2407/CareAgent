...
<b>Clinical Rationale:</b> {recommendations.get('clinical_rationale', 'No rationale provided.')}
                ...
                # Standalone Collapsible Dropdown ML Feature Contributions Panel (Lazy Loaded)
                with st.expander("👤 Machine Learning Feature Contributions"):
                    st.markdown("""
                    <p style="font-size:0.82rem; opacity:0.75; line-height:1.4; margin-bottom:15px;">
                        Inspect the mathematical contributions of patient-specific attributes to the Random Forest model's prediction. Red indicates factors increasing risk; Green indicates protective factors.
                    </p>
                    """, unsafe_allow_html=True)
                    
                    # On-demand loading checkbox
                    load_expl = st.checkbox("Generate local explainability report", key=f"load_expl_{selected_patient_id}")
                    if load_expl:
                        with st.spinner("Fetching explanation parameters from server..."):
                            try:
                                r = requests.get(f"{BACKEND_URL}/patients/{selected_patient_id}/explanations")
                                if r.status_code == 200:
                                    local_contribs_all = r.json().get("local_contributions", {})
                                    ...
                                    st.error("Failed to retrieve explanations from server.")
                            except Exception as e:
